...
# Auto parser discovery modules
try:
    from parser_discovery import generate_and_save_parser_registry, summarize_detected_json_types
    AUTO_PARSER_AVAILABLE = True
except Exception:
    generate_and_save_parser_registry = None
    summarize_detected_json_types = None
    AUTO_PARSER_AVAILABLE = False
...
GENERATED_PARSER_MAPPING_FILE = getattr(app_config, "GENERATED_PARSER_MAPPING_FILE", CACHE_DIR / "parser_mappings.generated.json")
PARSER_DISCOVERY_SAMPLE_LIMIT = getattr(app_config, "PARSER_DISCOVERY_SAMPLE_LIMIT", 200)
...
def load_record_groups(data_dir: Path):
    ...
    if RECORD_UNIT_LOADER_AVAILABLE:
        return group_record_units_by_family(data_dir, allowed_json_types=st.session_state.get("selected_json_types") or None)
...
def initialize_state():
    defaults = {
        ...
        "family_stats": [],
        "family_skipped": [],
        "parser_scan_result": None,
        "detected_json_type_rows": [],
        "selected_json_types": [],
        "active_criteria": [],
        "active_output": [],
        "active_advanced": {
...
def scan_dataset_and_generate_parser_registry():
    """Scan dataset, detect JSON types, generate parser candidates, save generated registry, and expose selection UI state."""
    if not AUTO_PARSER_AVAILABLE:
        st.warning("Auto parser discovery modules are not available.")
        return
    folder = Path(st.session_state.dataset_folder_input.strip())
    if not folder.exists():
        st.error("Dataset folder does not exist.")
        return
    result = generate_and_save_parser_registry(folder, sample_limit=PARSER_DISCOVERY_SAMPLE_LIMIT, recursive=True)
    rows = summarize_detected_json_types(result.get("detected", []))
    st.session_state.parser_scan_result = result
    st.session_state.detected_json_type_rows = rows
    ready_types = [r["json_type"] for r in rows if r.get("json_type") != "unknown" and r.get("avg_confidence", 0.0) >= 0.60]
    st.session_state.selected_json_types = ready_types
    st.session_state.schema_build_token += 1
    # Clear family stats so next detection uses newly generated registry and selected types.
    st.session_state.available_record_families = []
    st.session_state.family_stats = []
    st.session_state.family_skipped = []
...
render_brand()
st.title("AQF")
st.caption("A generated query form over hierarchical healthcare records, now with composition, EHR JSON ingestion, and auto parser discovery support.")
...
with st.sidebar:
    ...
    if st.button("Scan dataset / generate parser registry", key="scan_generate_parsers"):
        scan_dataset_and_generate_parser_registry()
        do_rerun()

    if st.session_state.get("detected_json_type_rows"):
        st.markdown("#### Detected JSON types")
        detected_type_options = [r["json_type"] for r in st.session_state.detected_json_type_rows]
        if not st.session_state.selected_json_types:
            st.session_state.selected_json_types = [t for t in detected_type_options if t != "unknown"]
        st.multiselect(
            "JSON types to include",
            detected_type_options,
            key="selected_json_types",
            help="AQF extracts record units only from the selected JSON parser types before building record families.",
        )
...
with st.expander("Dataset diagnostics", expanded=False):
    if st.session_state.get("detected_json_type_rows"):
        st.markdown("#### JSON type diagnostics")
        st.dataframe(pd.DataFrame(st.session_state.detected_json_type_rows), use_container_width=True)
        st.caption(f"Generated parser registry: {GENERATED_PARSER_MAPPING_FILE}")
